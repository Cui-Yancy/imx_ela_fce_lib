...
# The authentication tag is printed to stdout.
```
...
| `-v <hex>` | IV/nonce as hex string (CBC: 32 hex, CTR/GCM: 24 hex) |
| `-V <file>` | IV/nonce from binary file |
| `-h` | Show help |
...
- GCM encryption produces a 16-byte authentication tag.  For GCM decryption
  the tag must be provided by the caller for verification.
